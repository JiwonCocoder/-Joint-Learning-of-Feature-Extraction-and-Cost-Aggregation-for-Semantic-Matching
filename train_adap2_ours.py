...
from lib.model_train.model_train_adap2_ours import ImMatchNet
...
parser.add_argument('--result_model_fn', type=str, default=None, help='trained model filename')
parser.add_argument('--trainLog', type=str, default=None, help='trained model filename')
parser.add_argument('--result_model_dir', type=str, default=None, help='path to trained models folder')
parser.add_argument('--fe_finetune_params',  type=int, default=0, help='number of layers to finetune')
parser.add_argument('--temperature', type=float, default=None, help='pixelCT_temperature')
parser.add_argument('--occ_threshold_adap', type=float, default=None, help='occ_threshold')
parser.add_argument('--occ_threshold_agg', type=float, default=None, help='occ_threshold')
parser.add_argument('--Norm', type=str, default=None, help='noNorm | L2Norm | softmax')
...
print(args)
root_path = '/root/dataset2/ncnet_bmvc/'
train_name = f'{args.trainLog}_bsize_{args.batch_size}_lr_{args.lr}_temp_{args.temperature}_thres_ada_{args.occ_threshold_ada}_thres_agg_{args.occ_threshold_agg}'
save_path = os.path.join(root_path , train_name)
print(save_path )
...
def weak_loss(model ,batch ,args, writer_position, mode, normalization='softmax' ,alpha=30):
    ...
    corr_WTA, corr_NET, mask_B, index_B = model(batch, args, writer, writer_position, mode, label='pos')

    batch_size = corr_WTA.size(0)
    feature_size = corr_WTA.size(2)
    batch_size, ch, fs1, fs2, fs3, fs4 = corr_NET.size()
    nc_WTA_A_Bvec = corr_WTA.view(batch_size, fs1, fs2, fs3 * fs4).permute(0, 3, 1,
                                                                           2).contiguous()  # (B,B_S*B_S, A_S,A_S)
    nc_NET_A_Bvec = corr_NET.view(batch_size, fs1, fs2, fs3 * fs4).permute(0, 3, 1,
                                                                           2).contiguous()  # (B,B_S*B_S, A_S,A_S)
    ...
    corr_WTA, corr_NET, mask_B, index_B = model(batch, args, writer, writer_position, mode, label='neg')
    batch_size = corr_WTA.size(0)
    feature_size = corr_WTA.size(2)
...
loss_fn = lambda model, batch, args, writer_position, mode: weak_loss(model, batch, args, writer_position, mode,
...
def process_epoch(mode, epoch, model, loss_fn, optimizer, dataloader, batch_preprocessing_fn, writer, use_cuda=True,
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        loss_pixelCT_WTA_pos, loss_pixelCT_NET_pos, loss_pixelCT_WTA_neg, loss_pixelCT_NET_neg = loss_fn(model,
                                                                                                         tnf_batch,
                                                                                                         args,
                                                                                                         writer_position,
                                                                                                         mode)
        if writer_position % 9 == 0:
            writer.add_scalar('Loss_{}/loss_pixelCT_WTA_B_Avec_byWTA_pos'.format(mode), loss_pixelCT_WTA_pos[0],
                              writer_position)
            ...
            writer.add_scalar('Loss_{}/loss_pixelCT_NET_B_Avec_byWTA_pos'.format(mode), loss_pixelCT_NET_pos[0],
                              writer_position)
            writer.add_scalar('Loss_{}/loss_pixelCT_NET_B_Avec_byNET_pos'.format(mode), loss_pixelCT_NET_pos[1],
                              writer_position)
        ...
        if batch_idx % log_interval == 0:
            print(mode.capitalize() + ' Epoch: {} [{}/{} ({:.0f}%)]\t\tLoss: {:.6f}'.format(epoch, batch_idx,
                                                                                            len(dataloader),
                                                                                            100. * batch_idx / len(
                                                                                                dataloader), loss_np))
