...
from lib.model_eval.model_eval_ncnet_adap import ImMatchNet
...
save_path = './testLog_pixelCT_mask_MSE_lr7_19'
...
for i, batch in enumerate(dataloader):
    ...
    corr4d = model(batch, writer, batch_start_idx)
    ...
    print("meanNET_softmax", score_B[0,:,:])
    print("scores_B", scores_B)
    print("before(NET)",nc_B_Avec[0, int(index_B[0, 15, 15]), 15, 15])
    print("after_softmax(NET)",nc_B_Avec_norm[0, int(index_B[0, 15, 15]), 15, 15]) 
    print("after_L2norm(NET)",nc_B_Avec_L2norm[0, int(index_B[0, 15, 15]), 15, 15])      
