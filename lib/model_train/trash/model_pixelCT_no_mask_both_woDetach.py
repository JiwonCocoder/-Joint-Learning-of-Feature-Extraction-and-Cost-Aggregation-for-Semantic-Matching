...
class ImMatchNet(nn.Module):
    ...
    def forward(self, tnf_batch, writer, writer_position):
        ...
        nc_B_Avec_WTA = torch.nn.functional.softmax(nc_B_Avec_WTA, 1)
        ...
        corr4d_NET = self.NeighConsensus(corr4d)
        corr4d_NET = MutualMatching(corr4d_NET)
        nc_B_Avec2 = corr4d_NET.contiguous().view(batch_size, fs1 * fs2, fs3, fs4)  # [batch_idx,k_A,i_B,j_B]
        ...
        if writer_position % (4-1) == 0:
            ...
            writer.add_figure('img_grid_WTA/pixelCT_both_no_mask_lr5_{}'.format(writer_position), img_grid_WTA)
            writer.add_figure('img_grid_NET/pixelCT_both_no_mask_lr5_{}'.format(writer_position), img_grid_NET)
        if self.relocalization_k_size > 1:
