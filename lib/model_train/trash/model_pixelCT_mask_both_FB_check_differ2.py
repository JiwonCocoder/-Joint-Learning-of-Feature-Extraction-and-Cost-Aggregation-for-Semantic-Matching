...
class ImMatchNet(nn.Module):
    def __init__(self,
        ...
        self.threshold_WTA = [100,81,72,64,49,49,49,49]
        self.threshold_NET = [100,81,72,64,49,36,25,16]
        feature_H = 25
        feature_W = 25
        beta = 50
        kernel_sigma = 5
    ...
    def forward(self, tnf_batch, writer, writer_position, mode, label):
        ...
        nc_A_Bvec_WTA = torch.nn.functional.softmax(nc_A_Bvec_WTA, 1)

        nc_A_Bvec_WTA_norm = torch.nn.functional.softmax(nc_A_Bvec_WTA, 1)

        nc_B_Avec_WTA_base = corr4d_base.view(batch_size, fs1 * fs2, fs3, fs4)
        nc_B_Avec_WTA_base_norm = torch.nn.functional.softmax(nc_B_Avec_WTA_base, 1)




        # nc_B_Avec = torch.nn.functional.softmax(nc_B_Avec, 1)
        # nc_B_Avec = featureL2Norm(s elf.ReLU(nc_B_Avec))
        # compute matching scores
        scores_B, index_B = torch.max(nc_B_Avec_WTA_norm, dim=1)
        scores_WTA_B_noNorm, _ = torch.max(nc_B_Avec_WTA, dim=1)
        scores_WTA_B_L2Norm, _ = torch.max(nc_B_Avec_WTA_L2norm, dim=1)

        scores_A, index_A= torch.max(nc_A_Bvec_WTA_norm, dim=1)

        score_B_base, index_B_base = torch.max(nc_B_Avec_WTA_base_norm, dim =1 )
        ...
        if writer_position < 400:
            index_WTA = writer_position // 50
            occ_thresh = self.threshold_WTA[index_WTA]
        else:
            occ_thresh = 49
        ...
        nc_B_Avec2_norm = torch.nn.functional.softmax(nc_B_Avec2, 1)
        nc_B_Avec_NET_L2norm = featureL2Norm(nc_B_Avec2)

        nc_A_Bvec2_norm = torch.nn.functional.softmax(nc_A_Bvec2, 1)

        scores_NET_B, index_NET_B = torch.max(nc_B_Avec2_norm, dim=1)
        scores_NET_B_noNorm, _ = torch.max(nc_B_Avec2, dim=1)
        scores_NET_B_L2Norm, _ = torch.max(nc_B_Avec_NET_L2norm, dim=1)

        scores_NET_A, index_NET_A= torch.max(nc_A_Bvec2_norm, dim=1)
        index1D_NET_B = index_NET_B.view(batch_size, -1)
        index1D_NET_A = index_NET_A.view(batch_size, -1)
        # print("score_WTA", scores_B[0, 15, 15])
        # print("index_WTA_B", index_B[0, 15, 15])
        # print("index1D_WTA_B",index1D_B[0, 25*15+15] )
        # print("corr_score_WTA_B_norm is same as score_WTA", nc_B_Avec_WTA[0, index_B[0, 15, 15], 15, 15], scores_B[0, 15, 15])

        # print("score_NET", scores_NET_B[0, 15, 15])
        # print("index_NET_B", index_NET_B[0, 15, 15])
        # print("index1D_NET_B",index1D_NET_B[0, 25*15+15])
        # print("corr_score_NET_B is same as pixelCT", nc_B_Avec2[0, index_NET_B[0, 15, 15], 15, 15])
        # print("corr_score_NET_B_norm is same as score_NET", nc_B_Avec2_norm[0, index_NET_B[0, 15, 15], 15, 15], scores_NET_B[0, 15, 15])
        if writer_position % (10-1) == 0 and mode == 'train':

            writer.add_scalar('mean_score_{}/adap_before'.format(label), torch.mean(scores_WTA_B_noNorm[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('mean_score_{}/adap_softMax'.format(label), torch.mean(scores_B[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('mean_score_{}/adap_L2norm'.format(label), torch.mean(scores_WTA_B_L2Norm[0, :, :]).data.cpu().numpy(),
                              writer_position)                                                            
            writer.add_scalar('mean_score_{}/net_before'.format(label), torch.mean(scores_NET_B_noNorm[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('mean_score_{}/net_softMax'.format(label), torch.mean(scores_NET_B[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('mean_score_{}/net_L2norm'.format(label),torch.mean(scores_NET_B_L2Norm[0, :, :]).data.cpu().numpy(),
                              writer_position)   

            writer.add_scalar('max_score_{}/adap_before'.format(label), torch.max(scores_WTA_B_noNorm[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('max_score_{}/adap_softMax'.format(label), torch.max(scores_B[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('max_score_{}/adap_L2norm'.format(label), torch.max(scores_WTA_B_L2Norm[0, :, :]).data.cpu().numpy(),
                              writer_position)                                                            
            writer.add_scalar('max_score_{}/net_before'.format(label), torch.max(scores_NET_B_noNorm[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('max_score_{}/net_softMax'.format(label), torch.max(scores_NET_B[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('max_score_{}/net_L2norm'.format(label), torch.max(scores_NET_B_L2Norm[0, :, :]).data.cpu().numpy(),
                              writer_position)   
        #mask-FB_check (NET)
        ...
        if writer_position < 400:
            index_NET = writer_position // 50
            occ_thresh = self.threshold_NET[index_NET]
        else:
            occ_thresh = 16

        occ_NET_B_Avec = generate_mask(Flow2D_NET_B_Avec, Flow2D_NET_B_Avec_bw,
                                       occ_thresh)  # compute: feature_map-based
        occ_NET_A_Bvec = generate_mask(Flow2D_NET_A_Bvec, Flow2D_NET_A_Bvec_bw,
                                       occ_thresh)  # compute: feature_map-based

        # occ_NET_B_Avec = occ_NET_B_Avec.unsqueeze(1)
        # occ_NET_A_Bvec = occ_NET_A_Bvec.unsqueeze(1)

        masked_index_NET_B = occ_NET_B_Avec.type(torch.float16) * index_NET_B
        masked_index_NET_B = masked_index_NET_B.type(torch.int64)
        masked_index1D_NET_B = masked_index_NET_B.view(batch_size, -1)
        masked_Map2D_NET_B_Avec = unNormMap1D_to_NormMap2D(masked_index1D_NET_B)
        mask2D_NET_B_img = F.interpolate(input=occ_NET_B_Avec.unsqueeze(1).type(torch.float), scale_factor=16,
                                         mode='bilinear', align_corners=True)
        masked_index_WTA_B = occ_B_Avec.type(torch.float16) * index_B
        masked_index_WTA_B = masked_index_WTA_B.type(torch.int64)
        masked_index1D_WTA_B = masked_index_WTA_B.view(batch_size, -1)
        masked_Map2D_WTA_B_Avec = unNormMap1D_to_NormMap2D(masked_index1D_WTA_B)
        mask2D_WTA_B_img = F.interpolate(input=occ_B_Avec.unsqueeze(1).type(torch.float), scale_factor=16,
                                         mode='bilinear', align_corners=True)


        # print(writer_position)
        # if batch_size == 1:print("score", scores_NET_B[0,1,0,0])
        ...
        if writer_position % (10-1) == 0:
            ...
            img_grid_WTA_B_Avec = return_plot_test_map(tnf_batch['source_image'][0].unsqueeze(0),
                                                tnf_batch['target_image'][0].unsqueeze(0),
                                                Map2D_WTA_B_Avec_base[0].unsqueeze(0),
                                                Map2D_WTA_B_Avec[0].unsqueeze(0), scale_factor=16,
                                                plot_name='B_Avec_WTA')
            img_grid_mask_NET = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0),
                                                          tnf_batch['target_image'][0].unsqueeze(0),
                                                          Map2D_NET_B_Avec[0].unsqueeze(0),
                                                          masked_Map2D_NET_B_Avec[0].unsqueeze(0),
                                                          mask2D_NET_B_img[0].unsqueeze(0), scale_factor=16,
                                                          plot_name='AtoB_MAP_lr6')

            img_grid_mask_WTA = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0),
                                                          tnf_batch['target_image'][0].unsqueeze(0),
                                                          Map2D_WTA_B_Avec[0].unsqueeze(0),
                                                          masked_Map2D_WTA_B_Avec[0].unsqueeze(0),
                                                          mask2D_WTA_B_img[0].unsqueeze(0), scale_factor=16,
                                                          plot_name='AtoB_MAP_lr6')
            ...
            writer.add_figure('img_grid_base/pixelCT_mask_{}'.format(writer_position), img_grid_WTA_B_Avec)
            writer.add_figure('img_grid_WTA/pixelCT_mask_{}'.format(writer_position), img_grid_mask_WTA)
            writer.add_figure('img_grid_NET/pixelCT_mask_{}'.format(writer_position), img_grid_mask_NET)
        if self.relocalization_k_size > 1:
            delta4d = (max_i, max_j, max_k, max_l)
            return (corr4d, delta4d)
        else:
            return corr4d, corr4d_NET, \
                   [occ_B_Avec.unsqueeze(1), occ_NET_B_Avec.unsqueeze(1)], \
                   [index_B, index_NET_B]
                   # [masked_index_WTA_B, masked_index_NET_B]
