...
class ImMatchNet(nn.Module):
    def __init__(self,
        ...
        self.threshold_WTA = threshold[0]
        self.threshold_NET = threshold[1]
        feature_H = 25
        feature_W = 25
        beta = 50
        kernel_sigma = 5
    ...
    def forward(self, tnf_batch, writer, writer_position):
        ...
        mask2D_score_WTA_B = torch.zeros_like(scores_B).bool()
        mask2D_score_WTA_B[scores_B > self.threshold_WTA] = True
        mask2D_WTA_B = mask2D_score_WTA_B.unsqueeze(1) & occ_B_Avec.bool()
        mask2D_WTA_B = mask2D_WTA_B.type(torch.int8)

        masked_Map2D_WTA_B = torch.zeros_like(Map2D_WTA_B_Avec)
        masked_Map2D_WTA_B[:, 0, :, :] = mask2D_WTA_B.squeeze(1) * Map2D_WTA_B_Avec[:, 0, :, :]
        masked_Map2D_WTA_B[:, 1, :, :] = mask2D_WTA_B.squeeze(1) * Map2D_WTA_B_Avec[:, 1, :, :]
        # (show) mask
        ...
        print("mean_WTA:", torch.mean(scores_B[0,:,:]))
        ...
        print("mean_NET:", torch.mean(scores_NET_B[0,:,:]))
        ...
        if writer_position % 9 == 0:
            writer.add_scalar('corr/corr_base', nc_B_Avec_WTA_base[0, index_B[0, 15, 15], 15, 15].data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('corr/corr_adap_before', nc_B_Avec_WTA[0, index_B[0, 15, 15], 15, 15].data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('corr/corr_net_before', nc_B_Avec2[0, index_NET_B[0, 15, 15], 15, 15].data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('corr/mean_WTA', torch.mean(scores_B[0,:,:]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('corr/mean_NET', torch.mean(scores_NET_B[0,:,:]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('corr/MAX_WTA', torch.max(scores_B[0,:,:]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('corr/MAX_NET', torch.max(scores_NET_B[0,:,:]).data.cpu().numpy(),
                              writer_position)                                                                                            
        # mask-FB_check
        ...
        mask2D_score_NET_B = torch.zeros_like(scores_NET_B).bool()
        mask2D_score_NET_B[scores_NET_B > self.threshold_NET] = True

        mask2D_NET_B = mask2D_score_NET_B.unsqueeze(1) & occ_NET_B_Avec.bool()
        mask2D_NET_B = mask2D_NET_B.type(torch.int8)

        masked_Map2D_NET_B = torch.zeros_like(Map2D_NET_B_Avec)
        masked_Map2D_NET_B[:, 0, :, :] = mask2D_NET_B.squeeze(1) * Map2D_NET_B_Avec[:, 0, :, :]
        masked_Map2D_NET_B[:, 1, :, :] = mask2D_NET_B.squeeze(1) * Map2D_NET_B_Avec[:, 1, :, :]
        #change: not mean!! max!!
        Map2D_comb_B = torch.max(masked_Map2D_WTA_B, masked_Map2D_NET_B)
        masked_Map2D_comb_B = Map2D_comb_B
        # mask2D_B = torch.logical_or(mask2D_WTA_B, mask2D_NET_B)
        mask2D_B = mask2D_WTA_B.bool() | mask2D_NET_B.bool()
        # mask2D_B_intersection = torch.logical_and(mask2D_WTA_B, mask2D_NET_B)
        mask2D_B_intersection = mask2D_WTA_B.bool() & mask2D_NET_B.bool()
        masked_Map2D_comb_B_x = masked_Map2D_comb_B[:, 0, :, :].unsqueeze(1)
        masked_Map2D_comb_B_x[mask2D_B_intersection] = masked_Map2D_comb_B_x[mask2D_B_intersection] 
        masked_Map2D_comb_B_y = masked_Map2D_comb_B[:, 1, :, :].unsqueeze(1)
        masked_Map2D_comb_B_y[mask2D_B_intersection] = masked_Map2D_comb_B_y[mask2D_B_intersection] 
        ...
        mask2D_WTA_B_img = F.interpolate(input=mask2D_WTA_B.type(torch.float), scale_factor=16, mode='bilinear',
                                         align_corners=True)
        mask2D_NET_B_img = F.interpolate(input=mask2D_NET_B.type(torch.float), scale_factor=16,
        ...
        if writer_position % 9 == 0:
            ...
            img_grid = return_plot_test_map(tnf_batch['source_image'][0].unsqueeze(0),
            ...
            img_grid_mask_WTA = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0),
                                                          ...
                                                          masked_Map2D_WTA_B[0].unsqueeze(0),
            ...
            img_grid_mask_NET = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0),
                                                          ...
                                                          masked_Map2D_NET_B[0].unsqueeze(0),
                                                          mask2D_NET_B_img[0].unsqueeze(0), scale_factor=16,
                                                          plot_name='AtoB_MAP_lr6')
            img_grid_mask_both = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0),
                                                           tnf_batch['target_image'][0].unsqueeze(0),
                                                           Map2D_comb_B[0].unsqueeze(0),
                                                           masked_Map2D_comb_B[0].unsqueeze(0),
                                                           mask2D_B_img[0].unsqueeze(0), scale_factor=16,
                                                           plot_name='AtoB_MAP_lr6')
        ...
        if self.relocalization_k_size > 1:
            ...
            return corr4d, corr4d_NET, \
                   [mask2D_WTA_B, mask2D_NET_B, mask2D_B], \
                   [index_B.type(torch.int64), index_NET_B.type(torch.int64),
                    rescaled_unNormMap2D_NET.type(torch.int64)]
