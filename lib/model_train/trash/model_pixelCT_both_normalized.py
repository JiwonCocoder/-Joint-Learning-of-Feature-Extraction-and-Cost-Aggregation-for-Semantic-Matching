...
class ImMatchNet(nn.Module):
    ...
    def forward(self, tnf_batch, writer, writer_position):
        ...
        #(B, S, S)
        mask2D_score_WTA_B[scores_B>self.threshold] = True
        mask2D_score_WTA_B = mask2D_score_WTA_B.unsqueeze(1)
        mask2D_score_WTA_B_numpy = mask2D_score_WTA_B.detach().cpu().numpy()
        # warping Map
        ...
        mask2D_WTA_B = mask2D_score_WTA_B & occ_B_Avec.bool()

        # mask2D_WTA_B = torch.logical_and(mask2D_score_WTA_B, occ_B_Avec.bool())
        masked_index_WTA_B= mask2D_WTA_B.type(torch.float16)*index_B.unsqueeze(1)
        masked_Map2D_WTA_B =torch.zeros_like(Map2D_WTA_B_Avec)
        masked_Map2D_WTA_B[:,0,:,:] = mask2D_WTA_B.type(torch.float16).squeeze(1) * Map2D_WTA_B_Avec[:,0,:,:]
        masked_Map2D_WTA_B[:,1,:,:] = mask2D_WTA_B.type(torch.float16).squeeze(1) * Map2D_WTA_B_Avec[:, 1, :, :]
        #(show) mask
        ...
        #(B, S, S)
        mask2D_score_NET_B[scores_NET_B>self.threshold] = True
        mask2D_score_NET_B = mask2D_score_NET_B.unsqueeze(1)
        ...
        mask2D_NET_B = mask2D_score_NET_B & occ_NET_B_Avec.bool()

        # mask2D_NET_B = torch.logical_and(mask2D_score_NET_B, occ_NET_B_Avec.bool())
        mask2D_NET_B_numpy = mask2D_NET_B.detach().cpu().numpy()
        masked_index_NET_B = mask2D_NET_B.type(torch.float16) * index_NET_B.unsqueeze(1)

        masked_Map2D_NET_B = torch.zeros_like(Map2D_NET_B_Avec)
        masked_Map2D_NET_B[:, 0, :, :] = mask2D_NET_B.type(torch.float16).squeeze(1) * Map2D_NET_B_Avec[:, 0, :, :]
        masked_Map2D_NET_B[:, 1, :, :] = mask2D_NET_B.type(torch.float16).squeeze(1) * Map2D_NET_B_Avec[:, 1, :, :]

        masked_Map2D_comb_B = (masked_Map2D_WTA_B + masked_Map2D_NET_B)
        mask2D_B = mask2D_WTA_B | mask2D_NET_B
        mask2D_B_intersection = mask2D_WTA_B & mask2D_NET_B
        ...
        #show_plot
        mask2D_WTA_B_img = F.interpolate(input=mask2D_WTA_B.type(torch.float), scale_factor=16, mode='bilinear', align_corners=True)
        ...
        if writer_position % (4-1) == 0:
            ...
            img_grid_mask_WTA = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), masked_Map2D_WTA_B[0].unsqueeze(0),
                                            masked_Map2D_WTA_B[0].unsqueeze(0), mask2D_WTA_B_img[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            img_grid_mask_NET = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), masked_Map2D_NET_B[0].unsqueeze(0),
                                            masked_Map2D_NET_B[0].unsqueeze(0), mask2D_NET_B_img[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            img_grid_mask_both = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), masked_Map2D_comb_B[0].unsqueeze(0),
            ...
            writer.add_figure('adap_grid_base/pixelCT_both_neg_lr5_{}'.format(writer_position), img_grid)
            writer.add_figure('adap_grid_WTA/pixelCT_both_neg_lr5_{}'.format(writer_position), img_grid_mask_WTA)
            writer.add_figure('adap_grid_NET/pixelCT_both_neg_lr5_{}'.format(writer_position), img_grid_mask_NET)
            writer.add_figure('adap_grid_both/pixelCT_both_neg_lr5_{}'.format(writer_position), img_grid_mask_both)
        if self.relocalization_k_size > 1:
            delta4d = (max_i, max_j, max_k, max_l)
            return (corr4d, delta4d)
        else:
            return corr4d, corr4d_NET, mask2D_B, rescaled_unNormMap2D_NET
