...
class ImMatchNet(nn.Module):
    ...
    def forward(self, tnf_batch, writer, writer_position):
        ...
        masked_index_WTA_B= mask2D_WTA_B.type(torch.float16)*index_B
        ...
        corr4d_NET = featureL2Norm(self.ReLU(corr4d_NET))
        ...
        masked_index_NET_B = mask2D_NET_B.type(torch.float16) * index_NET_B

        masked_index_B = (masked_index_WTA_B + masked_index_NET_B)
        masked_index_B_numpy = masked_index_B.detach().cpu().numpy()
        mask2D_B = torch.logical_or(mask2D_WTA_B, mask2D_NET_B)
        mask2D_B = mask2D_B.unsqueeze(1)
        mask2D_B_intersection = torch.logical_and(mask2D_WTA_B, mask2D_NET_B)
        masked_index_B[mask2D_B_intersection] = masked_index_B[mask2D_B_intersection]/2
        masked_index_B = masked_index_B.type(torch.int64)
        ...
        #show_plot
        masked_index1D_NET_B = masked_index_B.view(batch_size, -1)
        masked_Map2D_B_Avec = unNormMap1D_to_NormMap2D(masked_index1D_NET_B)
        mask2D_WTA_B_img = F.interpolate(input=mask2D_WTA_B.unsqueeze(1).type(torch.float), scale_factor=16, mode='bilinear', align_corners=True)
        mask2D_NET_B_img = F.interpolate(input=mask2D_NET_B.unsqueeze(1).type(torch.float), scale_factor=16,
        ...
        if writer_position % (4-1) == 0:
            ...
            # img_grid = return_plot_test_map(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), Map2D_WTA_B_Avec[0].unsqueeze(0),
            #                                 masked_Map2D_B_Avec[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            img_grid_mask_WTA = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), Map2D_WTA_B_Avec[0].unsqueeze(0),
                                            Map2D_WTA_B_Avec[0].unsqueeze(0), mask2D_WTA_B_img[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            img_grid_mask_NET = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), Map2D_NET_B_Avec[0].unsqueeze(0),
                                            Map2D_NET_B_Avec[0].unsqueeze(0), mask2D_NET_B_img[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            img_grid_mask_both = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), masked_Map2D_B_Avec[0].unsqueeze(0),
                                            masked_Map2D_B_Avec[0].unsqueeze(0), mask2D_B_img[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            ...
            writer.add_figure('adap_grid_WTA/pixelCT_both_neg_lr7_{}'.format(writer_position), img_grid_mask_WTA)
            writer.add_figure('adap_grid_NET/pixelCT_both_neg_lr7_{}'.format(writer_position), img_grid_mask_NET)
            writer.add_figure('adap_grid_both/pixelCT_both_neg_lr7_{}'.format(writer_position), img_grid_mask_both)
        if self.relocalization_k_size > 1:
            delta4d = (max_i, max_j, max_k, max_l)
            return (corr4d, delta4d)
        else:
            return corr4d, corr4d_NET, mask2D_B, masked_index_B
