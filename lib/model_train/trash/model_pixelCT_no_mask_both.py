...
def IndexMap2D_to_NormedMap2D(index_2D):
    b, h, w = index_2D.size()
    grid_x = index_2D % w
...
class NeighConsensus(torch.nn.Module):
    ...
    def forward(self, x):
        if self.symmetric_mode:
            ...
            x = self.conv(x) + self.conv(x.permute(0, 1, 4, 5, 2, 3).contiguous()).permute(0, 1, 4, 5, 2, 3).contiguous()
...
def MutualMatching(corr4d):
    ...
    corr4d_B = corr4d_B.view(batch_size, 1, fs1, fs2, fs3, fs4).contiguous()
    corr4d_A = corr4d_A.view(batch_size, 1, fs1, fs2, fs3, fs4).contiguous()
...
def index2DMap_to_Norm2DMap(index2D_Map):
    ...
    grid_x = (grid_x.float() / (w) - 0.5) * 2
    grid_y = index2D_Map // w
    grid_y = (grid_y.float() / (h) - 0.5) * 2
    grid_x = grid_x.unsqueeze(1)  # b x 1 x h x w
    grid_y = grid_y.unsqueeze(1)



    grid = torch.cat((grid_x.permute(0, 2, 3, 1).contiguous(), grid_y.permute(0, 2, 3, 1).contiguous()),
                     3)
...
class find_correspondence(nn.Module):
    ...
    def kernel_soft_argmax(self, corr):
        ...
        # corr = self.apply_gaussian_kidxernel(corr, sigma=self.kernel_sigma)
...
class ImMatchNet(nn.Module):
    ...
    def forward(self, tnf_batch, writer, writer_position, mode, label):
        ...
        nc_A_Bvec_WTA = corr4d.view(batch_size, fs1, fs2, fs3 * fs4).permute(0, 3, 1,2).contiguous()  #
        ...
        nc_B_Avec_WTA_base = torch.nn.functional.softmax(nc_B_Avec_WTA_base, 1)
        ...
        index1D_B = index_B.contiguous().view(batch_size, -1)
        index1D_A = index_A.contiguous().view(batch_size, -1)
        index1D_B_base = index_B_base.view(batch_size, -1)

        Map2D_WTA_B_Avec = unNormMap1D_to_NormMap2D(index1D_B)  # (B,2,S,S)
        # Map2D_WTA_B_Avec= index2DMap_to_Norm2DMap(index_B)
        ...
        nc_A_Bvec2 = corr4d_NET.view(batch_size, fs1, fs2, fs3 * fs4).permute(0, 3, 1,2).contiguous()
        ...
        # Map2D_NET_B_Avec= index2DMap_to_Norm2DMap(index_NET_B)

        index1D_NET_B = index_NET_B.view(batch_size, -1).contiguous()
        index1D_NET_A = index_NET_A.view(batch_size, -1).contiguous()
        ...
        # print("corr_score_WTA_B is same as pixelCT", nc_B_Avec_WTA[0, index_B[0, 15, 15], 15, 15])
        # print("corr_score_NET_B_norm is same as score_WTA", nc_B_Avec_WTA[0, index_B[0, 15, 15], 15, 15], scores_B[0, 15, 15])
        ...
        if writer_position % (10-1) == 0 and mode == 'train':
            ...
            img_grid_WTA = return_plot_test_map(tnf_batch['source_image'][0].unsqueeze(0), tnf_batch['target_image'][0].unsqueeze(0), Map2D_WTA_B_Avec_base[0].unsqueeze(0),
                                            Map2D_WTA_B_Avec[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            img_grid_NET = return_plot_test_map(tnf_batch['source_image'][0].unsqueeze(0),
                                            tnf_batch['target_image'][0].unsqueeze(0),
                                            Map2D_WTA_B_Avec[0].unsqueeze(0),
                                            Map2D_NET_B_Avec[0].unsqueeze(0), scale_factor=16, plot_name='AtoB_MAP_lr6')
            ...
            writer.add_figure('img_grid_WTA/pixelCT_both_{}'.format(writer_position), img_grid_WTA)
            writer.add_figure('img_grid_NET/pixelCT_both_{}'.format(writer_position), img_grid_NET)
        ...
        if self.relocalization_k_size > 1:
            ...
            return corr4d, corr4d_NET, index_B, index_NET_B
