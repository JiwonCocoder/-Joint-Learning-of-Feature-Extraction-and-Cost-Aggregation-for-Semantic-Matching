...
class ImMatchNet(nn.Module):
    ...
    def forward(self, tnf_batch, writer, writer_position, mode, label):
        ...
        corr4d_MM = MutualMatching(corr4d)
        corr4d_NET = self.NeighConsensus(corr4d_MM)
        ...
        if writer_position % (10-1) == 0 and mode == 'train':
            ...
            writer.add_figure('adap_grid_{}/adap_NET_{}'.format(label, writer_position), img_grid)
            writer.add_scalar('mean_score_{}/adap_before'.format(label), torch.mean(scores_WTA_B_noNorm[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('mean_score_{}/adap_softMax'.format(label), torch.mean(scores_WTA_B[0, :, :]).data.cpu().numpy(),
                              writer_position)
            ...
            writer.add_scalar('max_score_{}/adap_before'.format(label), torch.max(scores_WTA_B_noNorm[0, :, :]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('max_score_{}/adap_softMax'.format(label), torch.max(scores_WTA_B[0, :, :]).data.cpu().numpy(),
                              writer_position)
