...
from lib.matching_model import unNormMap1D_to_NormMap2D, NormMap2D_to_unNormMap2D, \
    unnormalise_and_convert_mapping_to_flow, generate_mask, warp_with_mask
from lib.showPlot import plot_test_map, plot_test_flow, warpImg_fromMap, warpImg_fromMap2, matplotlib_imshow, \
    return_plot_test_map, get_img_from_fig, plot_test_map_mask_img
...
def calCorr(corr4d, direction):
    ...
    if direction == "B_Avec":
        corr3d = corr4d.view(batch_size, fs1 * fs2, fs3, fs4)  # [batch_idx,k_A,i_B,j_B]

    elif direction == "A_Bvec":
        corr3d = corr4d.view(batch_size, fs1, fs2, fs3 * fs4).permute(0, 3, 1, 2)  # [batch_idx,k_B,i_A,j_B]

    score["noNorm"], index["noNorm"] = torch.max(corr3d, dim=1)
    score["L2Norm"], index["L2Norm"] = torch.max(featureL2Norm(corr3d), dim=1)
    score["softmax"], index["softmax"] = torch.max(torch.nn.functional.softmax(corr3d, 1), dim=1)

    return score, index


def calOcc(index_B, index_A, occ_thresh):
    batch_size, _, _ = index_B.size() #(B, 2, H, W)

    index1D_B = index_B.view(batch_size, -1)
    index1D_A = index_A.view(batch_size, -1)

    Map2D_B_Avec = unNormMap1D_to_NormMap2D(index1D_B)  # (B,2,S,S)
    Map2D_A_Bvec = unNormMap1D_to_NormMap2D(index1D_A)  # (B,2,S,S)

    # mask-FB_check (WTA)
    Flow2D_B_Avec = unnormalise_and_convert_mapping_to_flow(Map2D_B_Avec)
    Flow2D_A_Bvec = unnormalise_and_convert_mapping_to_flow(Map2D_A_Bvec)
    Flow2D_B_Avec_bw = nn.functional.grid_sample(Flow2D_A_Bvec, Map2D_B_Avec.permute(0, 2, 3, 1))
    Flow2D_A_Bvec_bw = nn.functional.grid_sample(Flow2D_B_Avec, Map2D_A_Bvec.permute(0, 2, 3, 1))

    occ_B_Avec = generate_mask(Flow2D_B_Avec, Flow2D_B_Avec_bw, occ_thresh)  # compute: feature_map-based
    occ_A_Bvec = generate_mask(Flow2D_A_Bvec, Flow2D_A_Bvec_bw, occ_thresh)  # compute: feature_map-based
    # occ_B_Avec = occ_B_Avec.unsqueeze(1)
    # occ_A_Bvec = occ_A_Bvec.unsqueeze(1)
    return occ_B_Avec, occ_A_Bvec

def writer_scores(scores_list, label, writer, writer_position):
    name_list = ["base, adap, agg"]
    key_list = ["noNorm", "L2Norm", "softmax"]
    for key in key_list:
        for i, name in enumerate(name_list):
            # ex. mean_score_pos/base_noNorm
            writer.add_scalar('mean_score_{}/{}_{}'.format(label, name, key),
                              torch.mean(scores_list[i][key]).data.cpu().numpy(),
                              writer_position)
            writer.add_scalar('max_score_{}/{}_{}'.format(label, name, key),
                              torch.max(scores_list[i][key]).data.cpu().numpy(),
                              writer_position)
...
class ImMatchNet(nn.Module):
    ...
    def forward(self, tnf_batch, args, writer, writer_position, mode, label):
        # feature extraction
        ...
        adap_feature_A_L2norm = featureL2Norm(adap_feature_A)
        adap_feature_B_L2norm = featureL2Norm(adap_feature_B)
        feature_A_WTA_L2norm = featureL2Norm(feature_A)
        feature_B_WTA_L2norm = featureL2Norm(feature_B)
        print(adap_feature_B_L2norm.size())
        if self.half_precision:
            feature_A = feature_A.half()
            feature_B = feature_B.half()
        # feature correlation

        corr4d_adap = self.FeatureCorrelation(adap_feature_A_L2norm, adap_feature_B_L2norm)
        corr4d_base = self.FeatureCorrelation(feature_A_WTA_L2norm, feature_B_WTA_L2norm)
        # do 4d maxpooling for relocalization
        if self.relocalization_k_size > 1:
            corr4d_adap, max_i, max_j, max_k, max_l = maxpool4d(corr4d_adap, k_size=self.relocalization_k_size)

        # run match processing model
        corr4d_agg = MutualMatching(corr4d_adap.detach())
        corr4d_agg = self.NeighConsensus(corr4d_agg)
        corr4d_agg = MutualMatching(corr4d_agg)

        batch_size, ch, fs1, fs2, fs3, fs4 = corr4d_adap.size()
        scores_B_Avec_base, index_B_Avec_base = calCorr(corr4d_base, direction='B_Avec')
        scores_B_Avec_adap, index_B_Avec_adap = calCorr(corr4d_adap, direction='B_Avec')
        scores_A_Bvec_adap, index_A_Bvec_adap = calCorr(corr4d_adap, direction='A_Bvec')
        scores_B_Avec_agg, index_B_Avec_agg = calCorr(corr4d_agg, direction='B_Avec')
        scores_A_Bvec_agg, index_A_Bvec_agg = calCorr(corr4d_agg, direction='A_Bvec')
        occ_thresh_adap = args.occ_threshold_adap
        occ_thresh_agg = args.occ_threshold_agg
        occ_B_Avec_adap, occ_A_vec_adap = calOcc(index_B_Avec_adap[args.Norm], index_A_Bvec_adap[args.Norm], occ_thresh_adap)
        occ_B_Avec_agg, occ_A_vec_agg = calOcc(index_B_Avec_agg[args.Norm], index_A_Bvec_agg[args.Norm], occ_thresh_agg)

        if writer_position % (10 - 1) == 0 and mode == 'train':
            #label = pos, neg
            writer_scores([scores_B_Avec_base, scores_B_Avec_adap, scores_B_Avec_agg],
                          label, writer, writer_position)

        normed_index_B_Avec_base = unNormMap1D_to_NormMap2D(index_B_Avec_base[args.Norm].view(batch_size, -1))
        normed_index_B_Avec_adap = unNormMap1D_to_NormMap2D(index_B_Avec_adap[args.Norm].view(batch_size, -1))
        mask_img_B_Avec_adap = F.interpolate(input=occ_B_Avec_adap.unsqueeze(1).type(torch.float),
                                      scale_factor= 16,
                                      mode = 'bilinear',
                                      align_corners= True)
        normed_index_B_Avec_agg = unNormMap1D_to_NormMap2D(index_B_Avec_agg[args.Norm].view(batch_size, -1))
        mask_img_B_Avec_agg = F.interpolate(input=occ_B_Avec_agg.unsqueeze(1).type(torch.float),
                                      scale_factor= 16,
                                      mode = 'bilinear',
                                      align_corners= True)
        ...
        if writer_position % (10 - 1) == 0:

            img_base_to_adap = return_plot_test_map(tnf_batch['source_image'][0].unsqueeze(0),
                                                       tnf_batch['target_image'][0].unsqueeze(0),
                                                       normed_index_B_Avec_base[0].unsqueeze(0),
                                                       normed_index_B_Avec_adap[0].unsqueeze(0), scale_factor=16,
                                                       plot_name=writer_position)

            img_adap_to_masked_adap = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0),
                                                          tnf_batch['target_image'][0].unsqueeze(0),
                                                          normed_index_B_Avec_adap[0].unsqueeze(0),
                                                          mask_img_B_Avec_adap[0].unsqueeze(0), scale_factor=16,
                                                          plot_name=writer_position)

            img_agg_to_masked_agg = return_plot_test_map_mask(tnf_batch['source_image'][0].unsqueeze(0),
                                                          tnf_batch['target_image'][0].unsqueeze(0),
                                                          normed_index_B_Avec_agg[0].unsqueeze(0),
                                                          mask_img_B_Avec_agg[0].unsqueeze(0), scale_factor=16,
                                                          plot_name=writer_position)

            writer.add_figure('img_base_to_adap/{}'.format(writer_position), img_base_to_adap)
            writer.add_figure('img_adap_to_masked_adap/{}'.format(writer_position), img_adap_to_masked_adap)
            writer.add_figure('img_agg_to_masked_agg/{}'.format(writer_position), img_agg_to_masked_agg)


        if self.relocalization_k_size > 1:
            delta4d = (max_i, max_j, max_k, max_l)
            return (corr4d_adap, delta4d)
        else:
            return corr4d_adap, corr4d_agg, \
                   [occ_B_Avec_adap.unsqueeze(1), occ_B_Avec_agg.unsqueeze(1)], \
                   [index_B_Avec_adap[args.Norm], index_B_Avec_agg[args.Norm]]
