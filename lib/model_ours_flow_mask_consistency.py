...
from lib.matching_model import unNormMap1D_to_NormMap2D, unnormalise_and_convert_mapping_to_flow, generate_mask, warp_with_mask
from lib.showPlot import plot_test_map, plot_test_flow, plot_test_flow_mask, plot_test_map_mask_img, plot_test_map_mask_fm
...
class ImMatchNet(nn.Module):
    ...
    def forward(self, tnf_batch):
        ...
        nc_A_Bvec = corr4d.view(batch_size, fs1, fs2, fs3 * fs4).permute(0, 3, 1,2)  #

        #Correlation S2T (channel: tgt / spatial_grid: src)
        # b,c, h, w = feature_A.size()
        # feature_1 = feature_A.view(b, c, h * w) #src
        # feature_2 = feature_A.view(b, c, h * w) #tgt
        # corr = torch.bmm(feature_2.transpose(1,2), feature_1)
        # corr = corr.view(b, h*w, h, w)

        nc_B_Avec = torch.nn.functional.softmax(nc_B_Avec, 1) #from src(A) to tgt(B)
        nc_A_Bvec = torch.nn.functional.softmax(nc_A_Bvec, 1)

        # nc_B_Avec = featureL2Norm(self.ReLU(nc_B_Avec))
        # compute matching scores
        scores_B, index_B = torch.max(nc_B_Avec, dim=1)
        scores_A, index_A= torch.max(nc_A_Bvec, dim=1)
        # warping Map
        ...
        # plot_test_map(tnf_batch['source_image'], tnf_batch['target_image'], Map2D_WTA_B_Avec, Map2D_WTA_A_Bvec, scale_factor=16,plot_name='AtoB_MAP' )
        ...
        # plot_test_map_mask_fm(tnf_batch['source_image'], tnf_batch['target_image'], Map2D_WTA_B_Avec, Map2D_WTA_B_Avec,
        #                        occ_B_Avec, scale_factor=1, plot_name='AtoB_MAP')
        # plot_test_map_mask_fm(tnf_batch['target_image'], tnf_batch['source_image'], Map2D_WTA_A_Bvec, Map2D_WTA_A_Bvec,
        #                        occ_A_Bvec, scale_factor=1, plot_name='AtoB_MAP')
        ...
        nc_B_Avec2 = corr4d.view(batch_size, fs1 * fs2, fs3, fs4)  # [batch_idx,k_A,i_B,j_B]
        nc_A_Bvec2 = corr4d.view(batch_size, fs1, fs2, fs3 * fs4).permute(0, 3, 1,2)  #

        Map2D_NET_B_Avec, Flow2D_NET_B_Avec = self.find_correspondence(nc_B_Avec2)
        Map2D_NET_A_Bvec, Flow2D_NET_A_Bvec = self.find_correspondence(nc_A_Bvec2)
        # plot_test_map_mask_fm(tnf_batch['source_image'], tnf_batch['target_image'], Map2D_NET_B_Avec, Map2D_NET_B_Avec,
        #                        occ_B_Avec, scale_factor=1, plot_name='AtoB_MAP')
        # plot_test_map_mask_fm(tnf_batch['target_image'], tnf_batch['source_image'], Map2D_NET_A_Bvec, Map2D_NET_A_Bvec,
        #                        occ_A_Bvec, scale_factor=1, plot_name='AtoB_MAP')
        #
        # plot_test_map_mask_img(tnf_batch['source_image'], tnf_batch['target_image'], Map2D_NET_B_Avec, Map2D_NET_B_Avec, occ_B_Avec_img, scale_factor=16,plot_name='AtoB_MAP' )
        # plot_test_map_mask_img(tnf_batch['target_image'], tnf_batch['source_image'], Map2D_NET_A_Bvec, Map2D_NET_A_Bvec,
        #                    occ_A_Bvec_img, scale_factor=16, plot_name='BtoA_MAP')
        if self.relocalization_k_size > 1:
            delta4d = (max_i, max_j, max_k, max_l)
            return (corr4d, delta4d)
        else:
            return [Map2D_WTA_B_Avec, Map2D_WTA_A_Bvec], [Map2D_NET_B_Avec,Map2D_NET_A_Bvec ], [occ_B_Avec, occ_A_Bvec]
