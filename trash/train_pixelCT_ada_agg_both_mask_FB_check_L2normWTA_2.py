...
from lib.model_train.model_pixelCT_mask_both_FB_check_differ2 import ImMatchNet
...
# parser.add_argument('--result_model_fn', type=str, default='checkpoint_ada_agg_both_mask100_55_temp1_L2normWTA', help='trained model filename')
parser.add_argument('--result_model_fn', type=str, default='checkpoint_ada_agg_both_mask_differ_55_temp05_L2normWTA_2', help='trained model filename')
parser.add_argument('--result-model-dir', type=str, default='trained_models', help='path to trained models folder')
# parser.add_argument('--trainLog', type=str, default='trainLog_ada_agg_both_mask100_55_temp1_L2normWTA', help='trained model filename')
parser.add_argument('--trainLog', type=str, default='trainLog_ada_agg_both_mask_differ_55_temp05_L2normWTA_2', help='trained model filename')
...
def weak_loss(model,batch,writer_position, mode, normalization='softmax',alpha=30):
    ...
    corr_WTA , corr_NET, mask_B, index_B= model(batch, writer, writer_position, mode, label = 'pos')
    ...
    corr_WTA , corr_NET, mask_B, index_B= model(batch, writer, writer_position, mode, label = 'neg')
    batch_size = corr_WTA.size(0)
    feature_size = corr_WTA.size(2)
    ...
    loss_pixelCT_WTA_B_Avec_byNET_neg = calc_pixelCT(nc_WTA_A_Bvec, index_B[1], args.temperature)

    # print("NET")
    loss_pixelCT_NET_B_Avec_byWTA_neg = calc_pixelCT(nc_NET_A_Bvec, index_B[0], args.temperature)
    loss_pixelCT_NET_B_Avec_byNET_neg = calc_pixelCT(nc_NET_A_Bvec, index_B[1], args.temperature)
