...
from lib.model_train.model_pixelCT_mask_comb_FB_check_and_score import ImMatchNet
...
parser.add_argument('--result_model_fn', type=str, default='checkpoint_ada_agg_both_comb_score_lr55_temp007_thres50_16_3', help='trained model filename')
...
parser.add_argument('--threshold_WTA', type=float, default=0.003, help='pixelCT_threshold')
parser.add_argument('--threshold_NET', type=float, default=0.0016, help='pixelCT_threshold')
...
print(args)
save_path = 'trainLog_ada_agg_both_comb_score_lr55_temp007_thres50_16_3'
...
model = ImMatchNet(use_cuda=use_cuda,
                   ...
                   threshold = [args.threshold_WTA, args.threshold_NET])
torch.manual_seed(1)
if use_cuda:
    print('torch cuda manual seed used =========================')
    torch.cuda.manual_seed(1)
...
def weak_loss(model,batch,writer_position, normalization='softmax',alpha=30):
    ...
    print("--------strat------------")
    print("WTA")
    num_fgnd_WTA = mask_B[0].sum(dim=3, keepdim=True).sum(dim=2, keepdim=True).sum(dim=0, keepdim=True)
    num_fgnd_WTA = num_fgnd_WTA.item()
    num_fgnd_NET = mask_B[1].sum(dim=3, keepdim=True).sum(dim=2, keepdim=True).sum(dim=0, keepdim=True)
    num_fgnd_NET = num_fgnd_NET.item()
    num_fgnd_comb = mask_B[2].sum(dim=3, keepdim=True).sum(dim=2, keepdim=True).sum(dim=0, keepdim=True)
    num_fgnd_comb = num_fgnd_comb.item()
    if num_fgnd_WTA >0:
        loss_pixelCT_WTA_B_Avec_byWTA_pos = calc_pixelCT_mask(nc_WTA_A_Bvec, index_B[0], mask_B[0], args.temperature)
    else:
        loss_pixelCT_WTA_B_Avec_byWTA_pos  = torch.tensor(0, dtype = torch.float32)
    if num_fgnd_comb>0:
        loss_pixelCT_WTA_B_Avec_byComb_pos = calc_pixelCT_mask(nc_WTA_A_Bvec, index_B[2], mask_B[2], args.temperature)
    else:
        loss_pixelCT_WTA_B_Avec_byComb_pos = torch.tensor([0], dtype=torch.float32)
    #Aggre
    if num_fgnd_NET > 0:
        loss_pixelCT_NET_B_Avec_byNET_pos = calc_pixelCT_mask(nc_NET_A_Bvec, index_B[1], mask_B[1], args.temperature)
    else:
        loss_pixelCT_NET_B_Avec_byNET_pos  = torch.tensor(0, dtype = torch.float32)
    if num_fgnd_comb > 0:
        loss_pixelCT_NET_B_Avec_byComb_pos = calc_pixelCT_mask(nc_NET_A_Bvec, index_B[2], mask_B[2], args.temperature)
    else:
        loss_pixelCT_NET_B_Avec_byComb_pos  = torch.tensor(0, dtype = torch.float32)
        loss_pixelCT_NET_B_Avec_byComb_pos = loss_pixelCT_NET_B_Avec_byComb_pos.cuda()
    print("--------end------------")
    ...
    corr_WTA , corr_NET, index_WTA_B, index_NET_B = model(batch, writer, writer_position)
    batch_size = corr_WTA.size(0)
    feature_size = corr_WTA.size(2)
    ...
    # loss_pixelCT_WTA_B_Avec_byComb_neg = calc_pixelCT_mask(nc_WTA_A_Bvec, index_B[2], mask_B[2], args.temperature)
    loss_pixelCT_WTA_B_Avec_byWTA_neg= calc_pixelCT(nc_WTA_A_Bvec, index_B[0], args.temperature)
    loss_pixelCT_WTA_B_Avec_byComb_neg = calc_pixelCT(nc_WTA_A_Bvec, index_B[2], args.temperature)

    print("NET")
    loss_pixelCT_NET_B_Avec_byNET_neg = calc_pixelCT(nc_NET_A_Bvec, index_B[0], args.temperature)
    loss_pixelCT_NET_B_Avec_byComb_neg = calc_pixelCT(nc_NET_A_Bvec, index_B[1], args.temperature)
    ...
    return [loss_pixelCT_WTA_B_Avec_byWTA_pos, loss_pixelCT_WTA_B_Avec_byComb_pos],\
           [loss_pixelCT_NET_B_Avec_byNET_pos, loss_pixelCT_NET_B_Avec_byComb_pos],\
           [loss_pixelCT_WTA_B_Avec_byWTA_neg, loss_pixelCT_WTA_B_Avec_byComb_neg],\
           [loss_pixelCT_NET_B_Avec_byNET_neg, loss_pixelCT_NET_B_Avec_byComb_neg]
...
def process_epoch(mode,epoch,model,loss_fn,optimizer,dataloader,batch_preprocessing_fn, writer, use_cuda=True,log_interval=50):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if writer_position % 9 == 0:
            ...
            writer.add_scalar('Loss_{}/loss_pixelCT_WTA_B_Avec_byComb_pos'.format(mode), loss_pixelCT_WTA_pos[1],
                            writer_position)
            writer.add_scalar('Loss_{}/loss_pixelCT_NET_B_Avec_byNET_pos'.format(mode), loss_pixelCT_NET_pos[0], writer_position)
            writer.add_scalar('Loss_{}/loss_pixelCT_NET_B_Avec_byComb_pos'.format(mode), loss_pixelCT_NET_pos[1],
                            writer_position)
            writer.add_scalar('Loss_{}/loss_pixelCT_WTA_B_Avec_byWTA_neg'.format(mode), loss_pixelCT_WTA_neg[0],
                            writer_position)
            writer.add_scalar('Loss_{}/loss_pixelCT_WTA_B_Avec_byComb_neg'.format(mode), loss_pixelCT_WTA_neg[1],
                            writer_position)
            writer.add_scalar('Loss_{}/loss_pixelCT_NET_B_Avec_byNET_neg'.format(mode), loss_pixelCT_NET_neg[0],
                            writer_position)
            writer.add_scalar('Loss_{}/loss_pixelCT_NET_B_Avec_byComb_neg'.format(mode), loss_pixelCT_NET_neg[1],
                            writer_position)
        ...
        if writer_position % 9 == 0:
            writer.add_scalar('Loss_{}/score_pos_pixelCT_WTA', score_pos_pixelCT_WTA, writer_position)
            writer.add_scalar('Loss_{}/score_pos_pixelCT_NET', score_pos_pixelCT_NET, writer_position)
            writer.add_scalar('Loss_{}/score_neg_pixelCT_WTA', score_neg_pixelCT_WTA, writer_position)
            writer.add_scalar('Loss_{}/score_neg_pixelCT_NET', score_neg_pixelCT_NET, writer_position)
            writer.add_scalar('Loss_{}/score_pos_pixelCT', score_pos_pixelCT, writer_position)
            writer.add_scalar('Loss_{}/score_neg_pixelCT', score_neg_pixelCT, writer_position)
            writer.add_scalar('Loss_{}/score_both_pixelCT', loss, writer_position)
