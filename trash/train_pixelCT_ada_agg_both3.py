...
parser.add_argument('--result_model_fn', type=str, default='checkpoint_pixelCT_ada_agg_both_lr55_temp10_L2normWTA', help='trained model filename')
...
print(args)
save_path = 'trainLog_pixelCT_ada_agg_both_lr55_temp10_L2normWTA'
...
def weak_loss(model,batch,writer_position, mode, normalization='softmax',alpha=30):
    ...
    corr_WTA , corr_NET, index_WTA_B, index_NET_B = model(batch, writer, writer_position, mode, label = 'neg')
    batch_size = corr_WTA.size(0)
    feature_size = corr_WTA.size(2)
