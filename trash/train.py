...
parser.add_argument('--result_model_fn', type=str, default='checkpoint_base_pos', help='trained model filename')
...
def process_epoch(mode, epoch, model, loss_fn, optimizer, dataloader, batch_preprocessing_fn, writer, use_cuda=True,
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        writer.add_scalar('Loss_{}_{}/loss_nc'.format(epoch, mode), loss, batch_idx)
        epoch_loss += loss_np
        if mode == 'train':
            loss.backward()
            optimizer.step()
