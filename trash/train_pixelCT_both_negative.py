...
parser.add_argument('--result_model_fn', type=str, default='checkpoint_trainLog_pixelCT_both_negative_professor', help='trained model filename')
...
print(args)
save_path = 'trainLog_pixelCT_both_negative_professor'
...
def weak_loss(model,batch, mode, writer, writer_position, normalization='softmax',alpha=30):
    ...
    score_pos_pixelCT = loss_pixelCT_WTA_B_Avec + loss_pixelCT_NET_B_Avec
    ...
    loss_pixelCT_WTA_B_Avec = calc_pixelCT_mask(nc_WTA_B_Avec, masked_index_B_Avec, mask_B_Avec, args.temperature)
    loss_pixelCT_NET_B_Avec = calc_pixelCT_mask(nc_NET_B_Avec, masked_index_B_Avec, mask_B_Avec, args.temperature)
    score_neg_pixelCT = loss_pixelCT_WTA_B_Avec + loss_pixelCT_NET_B_Avec
    ...
    return score_pos_pixelCT, score_neg_pixelCT
loss_fn = lambda model,batch, mode, writer, writer_position : weak_loss(model,batch, mode, writer, writer_position, normalization='softmax')
...
def process_epoch(mode,epoch,model,loss_fn,optimizer,dataloader,batch_preprocessing_fn, writer, use_cuda=True,log_interval=50):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        loss = loss_fn(model, tnf_batch, mode, writer, writer_position)
