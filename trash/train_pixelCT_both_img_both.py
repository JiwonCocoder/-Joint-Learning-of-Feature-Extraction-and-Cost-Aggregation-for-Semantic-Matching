...
parser.add_argument('--result_model_fn', type=str, default='checkpoint_pixelCT_both_img_both_lr6', help='trained model filename')
...
print(args)
save_path = 'trainLog_pixelCT_both_img_both_lr6'
...
def weak_loss(model,batch,writer, writer_position, normalization='softmax',alpha=30):
    ...
    nc_NET_B_Avec = corr_NET.view(batch_size, feature_size * feature_size, feature_size,
                            feature_size)
    nc_NET_A_Bvec = corr_NET.view(batch_size, feature_size, feature_size, feature_size * feature_size).permute(0, 3, 1, 2)  #
    ...
    nc_NET_B_Avec = normalize(nc_NET_B_Avec)
    nc_NET_A_Bvec = normalize(nc_NET_A_Bvec)

    # compute matching scores
    scores_B, _ = torch.max(nc_NET_B_Avec, dim=1)
    scores_A, _ = torch.max(nc_NET_A_Bvec, dim=1)
    ...
    corr_WTA , corr_NET, mask_B_Avec, masked_index_B_Avec = model(batch, writer, writer_position)
    batch_size = corr_NET.size(0)
    feature_size = corr_NET.size(2)
    nc_B_Avec = corr_NET.view(batch_size, feature_size * feature_size, feature_size,
                            feature_size)  # [batch_idx,k_A,i_B,j_B]
    nc_A_Bvec = corr_NET.view(batch_size, feature_size, feature_size, feature_size * feature_size).permute(0, 3, 1, 2)  #
    ...
    loss = (loss_pixelCT_WTA_B_Avec + loss_pixelCT_NET_B_Avec)*0.01 + (score_neg - score_pos)
    ...
    writer.add_scalar('Loss_train/loss_masked_NET_img_pos', score_pos, writer_position)
    writer.add_scalar('Loss_train/loss_masked_NET_img_neg', score_neg, writer_position)
    writer.add_scalar('Loss_train/loss', loss, writer_position)
