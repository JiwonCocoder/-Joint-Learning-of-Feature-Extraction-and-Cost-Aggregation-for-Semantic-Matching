...
from lib.model_train.model_pixelCT_no_mask_both import ImMatchNet
...
parser.add_argument('--result_model_fn', type=str, default='checkpoint_pixelCT_ada_agg_both_lr55_temp5_L2normWTA', help='trained model filename')
...
parser.add_argument('--temperature', type=float, default=5, help='pixelCT_temperature')
...
print(args)
save_path = 'trainLog_pixelCT_ada_agg_both_lr55_temp5_L2normWTA'
...
def weak_loss(model,batch,writer_position, mode, normalization='softmax',alpha=30):
    ...
    corr_WTA , corr_NET, index_WTA_B, index_NET_B= model(batch, writer, writer_position, mode, label = 'pos')
    ...
    # print("before_L2norm(WTA)",nc_WTA_B_Avec[0, int(index_WTA_B[0, 15, 15]), 15, 15])
    # print("before_L2norm(NET)",nc_NET_B_Avec[0, int(index_NET_B[0, 15, 15]), 15, 15])
    # nc_WTA_B_Avec = featureL2Norm(nc_WTA_B_Avec)
    # nc_NET_B_Avec = featureL2Norm(nc_NET_B_Avec)
    # print("after_L2norm(WTA)",nc_WTA_B_Avec[0, int(index_WTA_B[0, 15, 15]), 15, 15])
    # print("after_L2norm(NET)",nc_NET_B_Avec[0, int(index_NET_B[0, 15, 15]), 15, 15])    

    # print("--------strat------------")
    # print("WTA")

    loss_pixelCT_WTA_B_Avec_byWTA_pos = calc_pixelCT(nc_WTA_A_Bvec, index_WTA_B, args.temperature)
    loss_pixelCT_WTA_B_Avec_byNET_pos = calc_pixelCT(nc_WTA_A_Bvec, index_NET_B, args.temperature)

    #Aggre
    loss_pixelCT_NET_B_Avec_byWTA_pos = calc_pixelCT(nc_NET_A_Bvec, index_WTA_B, args.temperature)
    loss_pixelCT_NET_B_Avec_byNET_pos = calc_pixelCT(nc_NET_A_Bvec, index_NET_B, args.temperature)
    print("--------end------------")

    #neg
    batch['source_image'] = batch['source_image'][np.roll(np.arange(b), -1), :]  # roll
    corr_WTA , corr_NET, index_WTA_B, index_NET_B = model(batch, writer, writer_position, model, label = 'neg')
    batch_size = corr_WTA.size(0)
    feature_size = corr_WTA.size(2)
    ...
    print("WTA")
    loss_pixelCT_WTA_B_Avec_byWTA_neg= calc_pixelCT(nc_WTA_A_Bvec, index_WTA_B, args.temperature)
    loss_pixelCT_WTA_B_Avec_byNET_neg = calc_pixelCT(nc_WTA_A_Bvec, index_NET_B, args.temperature)

    print("NET")
    loss_pixelCT_NET_B_Avec_byWTA_neg = calc_pixelCT(nc_NET_A_Bvec, index_WTA_B, args.temperature)
    loss_pixelCT_NET_B_Avec_byNET_neg = calc_pixelCT(nc_NET_A_Bvec, index_NET_B, args.temperature)
...
def process_epoch(mode,epoch,model,loss_fn,optimizer,dataloader,batch_preprocessing_fn, writer, use_cuda=True,log_interval=50):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        loss_pixelCT_WTA_pos, loss_pixelCT_NET_pos, loss_pixelCT_WTA_neg, loss_pixelCT_NET_neg = loss_fn(model, tnf_batch, writer_position, mode)
        ...
        # score_pos_pixelCT = score_pos_pixelCT_WTA + 0.01*score_pos_pixelCT_NET
        # score_neg_pixelCT = score_neg_pixelCT_WTA + 0.01*score_neg_pixelCT_NET
